...
QWEN_N_CTX = int(os.getenv("CLIPFORGE_QWEN_N_CTX", "16384"))  # was 32768: a 32k KV-cache (~3-4GB) plus the ~5GB Q4 model exceeds 8GB VRAM and spills to system RAM, making generation very slow. Each chunk is only ~4-5k tokens, so 16k is ample and keeps everything on-GPU.
